...
fn part1() -> u64 { let filename = "input.txt";
    ...
    reader.lines().for_each(|line| {
        ...
        if let Some(c) = left_compartment.iter().find(|item| {
            right_compartment.contains(item)
        }) {
            common_items.push(*c);
        };
...
fn part2() -> u64 { let filename = "input.txt";
    let file = File::open(filename).unwrap();
    let reader = BufReader::new(file);
    ...
    reader.lines().for_each(|line| {
        ...
        if current_group == 3 {
            if let Some(c) = groups.0.iter().find(|item| {
                return groups.1.contains(item) && groups.2.contains(item) }) {
                result += c
            };
            current_group = 1;
        } else {
